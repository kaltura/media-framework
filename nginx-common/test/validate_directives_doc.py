        ...
        if '{' in cur_line:
            ...
            if cmd_name in out:
                ...
                if prev[0] == module_type:
                    report_err('Warning: duplicate directive \"%s\", in %s:%s' % (cmd_name, path, line_num))
                prev_fields = prev[3]
...
def validate_syntax_args(cmd_name, syntax, handler, post):
    if handler == 'ngx_conf_set_enum_slot':
        ...
        if syntax != expected:
            report_err('Error: invalid syntax for %s, expected: %s, got: %s' % (cmd_name, expected, syntax))
        return

    if 'complex_value' in handler:
        if not syntax.endswith('expr'):
            report_err('Error: invalid syntax for %s, expected: expr, got: %s' % (cmd_name, syntax))
        return
...
for module in module_list:
    ...
    if len(undoc_cmds) > 0:
        report_err('Error: undocumented directives: %s' % undoc_cmds)
        print_undoc_commands(sorted([src_cmds[cmd][1:] + (cmd,) for cmd in undoc_cmds]))

    if len(non_exist_cmds) > 0:
        report_err('Error: directives missing from src: %s' % non_exist_cmds)
    ...
    for cmd_name in set(doc_cmds).intersection(set(src_cmds)):
        ...
        if exp_ctx != '' and exp_ctx != doc_ctx.replace('stream/server', 'server'):
            report_err('Error: invalid context for %s, expected: %s, got: %s' % (cmd_name, exp_ctx, doc_ctx))

        doc_syntax = doc_attrs['syntax'].strip('`')

        if 'NGX_CONF_BLOCK' in src_fields[1]:
            if not doc_syntax.endswith(' { ... }'):
                report_err('Error: syntax for %s missing block' % cmd_name)
                continue
            doc_syntax = doc_syntax[:-len(' { ... }')]
        else:
            if not doc_syntax.endswith(';'):
                report_err('Error: syntax for %s missing semicolon' % cmd_name)
                continue
            doc_syntax = doc_syntax[:-1]

        if not doc_syntax.startswith(cmd_name):
            report_err('Error: invalid syntax for %s: %s' % (cmd_name, doc_syntax))
            continue
