    ...
    uint32_t version)
{
    ...
    atom_size = ATOM_HEADER_SIZE + sizeof(trun_atom_t) + track->frame_count * sizeof(trun_video_frame_t);
...
static u_char*
mp4_fragment_write_audio_trun_atom(u_char* p, media_segment_track_t* track, uint32_t first_frame_offset)
{
    vod_list_part_t* part;
    input_frame_t* cur_frame;
    input_frame_t* last_frame;
    ...
    atom_size = ATOM_HEADER_SIZE + sizeof(trun_atom_t) + track->frame_count * sizeof(trun_audio_frame_t);
    ...
    uint32_t version)
{
    ...
    {
    case MEDIA_TYPE_VIDEO:
        p = mp4_fragment_write_video_trun_atom(p, track, first_frame_offset, version);
        break;

    case MEDIA_TYPE_AUDIO:
        p = mp4_fragment_write_audio_trun_atom(p, track, first_frame_offset);
        break;
    }
    return p;
}
...
static void
mp4_fragment_init_track(fragment_writer_state_t* state, media_segment_track_t* track)
{
    state->first_time = TRUE;
    state->first_frame_part = &track->frames.part;
    ...
    fragment_writer_state_t** result)
{
    ...
    mp4_fragment_init_track(state, track);
