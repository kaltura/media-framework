...
{
    media_segment_track_t* track = stream_state->track;

    stream_state->cur_frame_part = &track->frames.part;
    stream_state->cur_frame = track->frames.part.elts;
    stream_state->last_frame = stream_state->cur_frame + track->frames.part.nelts;
...
mp4_cbcs_encrypt_move_to_next_frame(mp4_cbcs_encrypt_stream_state_t* stream_state)
{
    for (;;)
    {
        ...
        if (stream_state->cur_frame_part->next != NULL)
        {
            ...
            stream_state->cur_frame = stream_state->cur_frame_part->elts;
            stream_state->last_frame = stream_state->cur_frame + stream_state->cur_frame_part->nelts;
            continue;
        }
...
mp4_cbcs_encrypt_video_init_track(mp4_cbcs_encrypt_video_stream_state_t* stream_state)
{
    ...
    media_segment_track_t* track = stream_state->base.track;
    vod_status_t rc;
...
mp4_cbcs_encrypt_video_write_buffer(void* context, u_char* buffer, uint32_t size)
{
    ...
    while (cur_pos < buffer_end)
    {
        ...
        case STATE_PACKET_COPY:
            ...
            if (!mp4_cbcs_encrypt_move_to_next_frame(&stream_state->base))
            {
                // finished all frames
...
mp4_cbcs_encrypt_audio_write_buffer(void* context, u_char* buffer, uint32_t size)
{
    ...
    while (cur_pos < buffer_end)
    {
        ...
        // finished a frame
        if (!mp4_cbcs_encrypt_move_to_next_frame(stream_state))
        {
            return mp4_cbcs_encrypt_flush(state);
        }
    }
    ...
    segment_writer_t** result)
{
    ...
    state = vod_alloc(request_context->pool,
        sizeof(*state) +
        sizeof(segment_writers[0]) * segment->track_count);
    if (state == NULL)
    {
        vod_log_debug0(VOD_LOG_DEBUG_LEVEL, request_context->log, 0,
