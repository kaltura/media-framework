    ...
    vod_str_t* response_header)
{
    ...
    *simulation_supported = hls_muxer_simulation_supported(media_segment, encryption_params);
    ...
    // init the id3 stream
    rc = hls_muxer_init_id3_stream(state, conf, media_segment, &init_streams_state);
    if (rc != VOD_OK)
    {
        return rc;
    }
