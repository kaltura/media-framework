...
def test(channelId=CHANNEL_ID):
    ...
    logTracker.assertContains('ngx_live_filler_set_channel: missing mandatory params (2)')
    ...
    logTracker.assertContains('ngx_live_filler_read_file: unknown preset "bad"')
    ...
    assertHttpError(lambda:
        nl.channel.update(NginxLiveChannel(id=channelId, preset='main', filler=
            ...
            404)
    logTracker.assertContains('ngx_live_filler_ready_handler: notif failed 404')
    ...
    logTracker.assertContains('ngx_live_filler_source_set: attempt to change filler from "__filler:main" to "bla:main"')
    ...
    logTracker.assertContains('ngx_live_filler_set_channel: missing mandatory params (1)')
    ...
    logTracker.assertContains('ngx_live_filler_write_file: unknown timeline "bad" in channel "__filler"')
    ...
    logTracker.assertContains('ngx_live_filler_write_file: timeline must have a single period')
    ...
    assertHttpError(lambda:
        nl.channel.update(NginxLiveChannel(id='empty2',
            filler=NginxLiveFiller(save=True, timeline_id='main'))), 400)
    logTracker.assertContains('ngx_live_filler_write_channel: no segments written')
